...
for cmd in gcloud kubectl jq psql node; do
...
for f in 00-schema-core.sql 01-schema-plaid.sql 02-schema-realestate.sql seed-checking.sql seed-debt.sql seed-realestate.sql; do
  if [[ ! -f "$SQL_DIR/$f" ]]; then
...
# 3b. Plaid schema → Checking & Savings + Debt Management
...
for i in $(seq 0 $((WORKSPACE_COUNT - 1))); do
  ...
  case "$DOMAIN_TYPE" in
    ...
    realestate)
      ...
      ;;
    null|"")
      log_info "No seed data for orchestrator: $WS_NAME"
      ;;
    *)
...
if [[ "$SKIP_K8S" == false ]]; then
  ...
  for i in $(seq 0 $((WORKSPACE_COUNT - 1))); do
    ...
    # Build the DATABASE_URL for Cloud SQL via internal proxy sidecar
