...
// ─── Domain Account Type Mapping ────────────────────────────────────────────
// Maps domain types to the Plaid account types they are authorized to access.
// This is the single source of truth for domain isolation at the data layer.
...
async function syncCheckingData(config) {
    ...
    // Determine which Plaid account types belong in this domain
    const allowedTypes = config.allowedAccountTypes
        || DOMAIN_ALLOWED_TYPES[config.domainType]
        || ['depository'];
    ...
    return withPool(config.databaseUrl, async (pool) => {
        ...
        const summary = { success: true, domain: config.domainType, allowedTypes };

        // 2. Fetch all accounts from Plaid, filter to domain-scoped types BEFORE storing
        const accountsRes = await plaid.accountsGet(config.accessToken);
        const allAccounts = accountsRes.data.accounts;
        const accounts = allAccounts.filter(a => allowedTypes.includes(a.type));
        const allowedAccountIds = new Set(accounts.map(a => a.account_id));

        summary.totalFromPlaid = allAccounts.length;
        summary.filteredToDomain = accounts.length;
        summary.excluded = allAccounts.length - accounts.length;

        // Only domain-scoped accounts are inserted — non-domain data never touches the DB
        ...
        // 3. Sync transactions — only store txns for domain-scoped accounts
        let cursor;
        if (config.itemId) {
        ...
        while (hasMore) {
            ...
            // Insert added transactions — SKIP any for non-domain accounts
            for (const txn of data.added) {
                if (!allowedAccountIds.has(txn.account_id)) { skippedCount++; continue; }
           ...
           ON CONFLICT (transaction_id) DO UPDATE SET
            ...
            // Update modified transactions — SKIP any for non-domain accounts
            for (const txn of data.modified) {
                if (!allowedAccountIds.has(txn.account_id)) { continue; }
                await pool.query(`INSERT INTO plaid_transactions
             (transaction_id, account_id, amount, date, name,
              merchant_name, category, payment_channel, pending, synced_at)
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9, NOW())
        ...
        summary.transactionsSkipped = skippedCount;
        return summary;
    });
}
...
function createGetBalancesHandler(config) {
    ...
    return async (_input, _ctx) => {
        return withPool(config.databaseUrl, async (pool) => {
            // Only query accounts matching this domain's allowed types
...
function createGetTransactionsHandler(config) {
    ...
    return async (input, _ctx) => {
        return withPool(config.databaseUrl, async (pool) => {
            ...
            // Only return transactions for domain-scoped accounts
            const typePlaceholders = allowedTypes.map((_, i) => `$${paramIdx + i}`).join(', ');
            conditions.push(`account_id IN (SELECT account_id FROM plaid_accounts WHERE type IN (${typePlaceholders}))`);
            params.push(...allowedTypes);
            paramIdx += allowedTypes.length;
