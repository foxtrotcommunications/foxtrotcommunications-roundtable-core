...
const tool = {
    ...
    description: 'Summarize income from plaid_transactions. Filters for negative amounts (Plaid convention: ' +
    ...
    async execute(args, _workspaceConfig = {}) {
        ...
        try {
            ...
            const conditions = ['amount < 0']; // negative = income
            const params = [];
            let idx = 1;
            if (account_id) {
      ...
      `;
            ...
            if (earliestDate && latestDate) {
                ...
                monthsSpanned = Math.max(1, (d2.getFullYear() - d1.getFullYear()) * 12 +
      ...
      `;
            ...
            const sources = bySourceResult.rows.map((r) => ({
                source: r.source,
            ...
            const deposits = depositsResult.rows.map((r) => ({
...
export default tool;
//# sourceMappingURL=getIncomeSummary%202.js.map