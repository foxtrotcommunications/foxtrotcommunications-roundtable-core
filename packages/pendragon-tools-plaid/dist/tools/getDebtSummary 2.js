...
const tool = {
    ...
    description: 'Get an aggregated debt overview: total debt, minimum payments, average interest rate, ' +
      ...
      `;
            ...
            const metadata = {
                coverage: {
                    ...
                    types: breakdown.map((b) => b.type),
                    hasData: summary.total_accounts > 0,
                    gaps: [],
                },
            };
            if (summary.total_accounts === 0)
                metadata.coverage.gaps.push('No liabilities found — Plaid may not support liabilities for this institution');
            const connections = JSON.parse(process.env.RT_CONNECTIONS || '[]');
            const coverageGaps = [...metadata.coverage.gaps];
            ...
            return {
                ...
                chart: {
                    ...
                    values: breakdown.map((b) => b.total_balance),
                },
                metadata: {
...
export default tool;
//# sourceMappingURL=getDebtSummary%202.js.map