...
const tool = {
    ...
    description: 'Calculate credit utilization per card and overall. Returns each credit account with ' +
    ...
    async execute(args, _workspaceConfig = {}) {
      ...
      `;
            ...
            if (cards.length === 0)
                metadata.coverage.gaps.push('No credit accounts found — user may not have linked credit cards');
            ...
            return {
                ...
                chart: {
                    labels: cards.map((c) => c.name || `****${c.mask}`),
                    values: cards.map((c) => c.utilization_pct),
                },
                metadata: {
...
export default tool;
//# sourceMappingURL=getCreditUtilization%202.js.map