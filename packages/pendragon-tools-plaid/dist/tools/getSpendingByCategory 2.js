...
const tool = {
    ...
    description: 'Aggregate spending from plaid_transactions grouped by category. ' +
    ...
    async execute(args, _workspaceConfig = {}) {
        ...
        try {
            ...
            const conditions = ['amount > 0']; // positive = spending
            const params = [];
            let idx = 1;
            if (account_id) {
      ...
      `;
            ...
            const transactionsScanned = categories.reduce((sum, c) => sum + c.count, 0);
...
export default tool;
//# sourceMappingURL=getSpendingByCategory%202.js.map