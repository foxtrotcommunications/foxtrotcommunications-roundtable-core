...
const tool = {
    name: 'get_cashflow',
    description: 'Compute cashflow over time from plaid_transactions. For each period (daily/weekly/monthly): ' +
      ...
      `;
            ...
            return {
                ...
                summary: {
                    ...
                    avg_monthly_net: granularity === 'monthly' && periods.length > 0
                        ? parseFloat((totalNet / periods.length).toFixed(2))
                        : null,
                },
                chart: {
                    labels: periods.map((p) => p.period),
                    income_values: periods.map((p) => p.income),
                    spending_values: periods.map((p) => p.spending),
                    net_values: periods.map((p) => p.net),
                },
                granularity,
...
export default tool;
//# sourceMappingURL=getCashflow%202.js.map