...
const tool = {
    name: 'get_balance',
    description: 'Get the current balance for a specific account or all accounts. Returns available, current, and limit balances with last sync time for freshness.',
    parameters: {
        type: 'object',
        properties: {
            account_id: {
    ...
    async execute(args, _workspaceConfig = {}) {
        ...
        `;
            ...
            const totalCurrent = balances.reduce((sum, b) => sum + (b.balance_current || 0), 0);
            const totalAvailable = balances.reduce((sum, b) => sum + (b.balance_available || 0), 0);
...
export default tool;
//# sourceMappingURL=getBalance%202.js.map