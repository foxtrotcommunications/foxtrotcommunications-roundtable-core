...
export async function evaluateGoalProgress(pool, goalId, domainType) {
    ...
    switch (domainType) {
        ...
        case 'savings': {
            ...
            if (goalType === 'spending_cap')
                return evaluateSpendingCapGoal(pool, goal);
            if (goalType === 'savings_target')
                return evaluateSavingsTargetGoal(pool, goal);
            return evaluateSavingsGoal(pool, goal);
        }
        case 'debt': {
            const debtGoalType = goal.goal_type || '';
            if (debtGoalType === 'spending_cap')
                return evaluateSpendingCapGoal(pool, goal);
...
// ─── Spending Cap Goals ─────────────────────────────────────────────────────
async function evaluateSpendingCapGoal(pool, goal) {
    ...
    const spendResult = await pool.query(`SELECT
    ...
    const lastMonthResult = await pool.query(`SELECT COALESCE(SUM(CASE WHEN amount > 0 THEN amount ELSE 0 END), 0) as total_spent
     FROM plaid_transactions
    ...
    const avgResult = await pool.query(`SELECT
       COALESCE(SUM(CASE WHEN amount > 0 THEN amount ELSE 0 END), 0) as total_spent,
       COUNT(DISTINCT date_trunc('month', date)) as months_counted
     FROM plaid_transactions
...
async function evaluateSavingsTargetGoal(pool, goal) {
    ...
    const focusMerchants = params.focus_merchants || [];
    ...
    const baselineResult = await pool.query(`SELECT
       COALESCE(SUM(CASE WHEN amount > 0 THEN amount ELSE 0 END), 0) as total_spent,
       COUNT(DISTINCT date_trunc('month', date)) as months_counted
     FROM plaid_transactions
    ...
    const currentResult = await pool.query(`SELECT COALESCE(SUM(CASE WHEN amount > 0 THEN amount ELSE 0 END), 0) as total_spent
     FROM plaid_transactions
     WHERE date >= date_trunc('month', CURRENT_DATE)
       AND pending = false
       AND amount > 0
    ...
    const topMerchantsResult = await pool.query(`SELECT merchant_name, COUNT(*) as txn_count, SUM(amount) as total
     FROM plaid_transactions
     WHERE date >= NOW() - INTERVAL '3 months'
       AND pending = false
       AND amount > 0
