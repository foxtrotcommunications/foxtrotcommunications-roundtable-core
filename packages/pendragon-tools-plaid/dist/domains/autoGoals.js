...
async function computeSmartTarget(databaseUrl, domainType, template) {
    ...
    return withPool(databaseUrl, async (pool) => {
        switch (domainType) {
            ...
            case 'savings': {
                ...
                try {
                    const result = await pool.query(`SELECT COALESCE(SUM(CASE WHEN amount > 0 THEN amount ELSE 0 END), 0) as total_expenses,
             ...
             WHERE date >= NOW() - INTERVAL '3 months' AND pending = false`);
                    ...
                    const coverageMonths = template.parameters.months_coverage || 3;
            ...
            case 'debt': {
                ...
                try {
                    const result = await pool.query(`SELECT COALESCE(SUM(balance_current), 0) as total_debt FROM plaid_accounts`);
            ...
            case 'retirement': {
                ...
                try {
                    const result = await pool.query(`SELECT COALESCE(SUM(institution_value), 0) as total_value FROM plaid_holdings`);
 ...
 * Called after sync completes. Each domain MUST have at least one goal.
 */
export async function ensureDefaultGoals(config) {
    const { domainType, databaseUrl } = config;
    try {
        const hasGoals = await withPool(databaseUrl, async (pool) => {
            const result = await pool.query(`SELECT COUNT(*) as count FROM domain_goals WHERE status = 'active'`);
...
//# sourceMappingURL=autoGoals.js.map