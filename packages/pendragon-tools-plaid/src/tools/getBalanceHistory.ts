...
const tool: Tool = {
  ...
  description:
    'Reconstruct balance history for an account by applying transactions backward from the current balance. Returns chart-ready data with labels and values. Supports daily, weekly, or monthly granularity.',
  parameters: {
    type: 'object',
    properties: {
      account_id: {
  ...
  async execute(args: any, _workspaceConfig: any = {}) {
    ...
    try {
      ...
      const values: number[] = new Array(labels.length);
