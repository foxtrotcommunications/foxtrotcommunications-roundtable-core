...
async function syncDebtData(config: PlaidPluginConfig): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      if (liabilities.credit) {
        for (const cc of liabilities.credit) {
          const primaryApr = cc.aprs?.find((a: any) => a.apr_type === 'purchase_apr');
          await pool.query(
...
function createGetLiabilitiesHandler(config: PlaidPluginConfig): CapabilityHandler {
  return async (_input, _ctx) => {
    return withPool(config.databaseUrl, async (pool) => {
      const { rows } = await pool.query(
        `SELECT l.*, a.name AS account_name, a.mask, a.subtype,
...
function createGetDebtSummaryHandler(config: PlaidPluginConfig): CapabilityHandler {
  return async (_input, _ctx) => {
    return withPool(config.databaseUrl, async (pool) => {
...
function createGetCreditUtilizationHandler(config: PlaidPluginConfig): CapabilityHandler {
  return async (_input, _ctx) => {
    return withPool(config.databaseUrl, async (pool) => {
      const { rows } = await pool.query(
...
export function registerDebtTools(registry: ToolRegistry, config: PlaidPluginConfig): void {
  registry.register('plaid_sync', {
    name: 'plaid_sync',
    description:
      'Sync debt data from Plaid into this workspace\'s database. ' +
...
export function registerDebtCapabilities(registry: CapabilityRegistry, config: PlaidPluginConfig): void {
  // 1. Get balances (credit card and loan account balances)
  registry.register({
  ...
  // 2. Get transactions (credit card charges, loan payments — itemized by merchant/category)
  registry.register({
