...
function makeGetPropertySummary(databaseUrl: string): CapabilityHandler {
  return async (_input: Record<string, unknown>) => {
    return withPool(databaseUrl, async (pool) => {
      ...
      return {
        ...
        totalValue: result.rows.reduce((s: number, r: any) => s + parseFloat(r.current_value || 0), 0),
        totalEquity: result.rows.reduce((s: number, r: any) => s + parseFloat(r.equity || 0), 0),
        totalMortgageBalance: result.rows.reduce((s: number, r: any) => s + parseFloat(r.mortgage_balance || 0), 0),
...
function makeGetMortgageDetails(databaseUrl: string): CapabilityHandler {
  return async (input: Record<string, unknown>) => {
    return withPool(databaseUrl, async (pool) => {
      ...
      return {
        ...
        totalBalance: result.rows.reduce((s: number, r: any) => s + parseFloat(r.current_balance || 0), 0),
        totalMonthlyPayment: result.rows.reduce((s: number, r: any) => s + parseFloat(r.monthly_payment || 0), 0),
        weightedAvgRate: result.rows.length > 0
          ? result.rows.reduce((s: number, r: any) => s + parseFloat(r.interest_rate) * parseFloat(r.current_balance), 0) /
            result.rows.reduce((s: number, r: any) => s + parseFloat(r.current_balance || 0), 0)
          : 0,
      };
    });
  };
...
function makeGetEquityAnalysis(databaseUrl: string): CapabilityHandler {
  return async (_input: Record<string, unknown>) => {
    return withPool(databaseUrl, async (pool) => {
      await pool.query(PROPERTY_SCHEMA);
      await pool.query(SEED_DATA);
      ...
      const totalValue = result.rows.reduce((s: number, r: any) => s + parseFloat(r.current_value || 0), 0);
      const totalEquity = result.rows.reduce((s: number, r: any) => s + parseFloat(r.equity || 0), 0);
      const totalPurchasePrice = result.rows.reduce((s: number, r: any) => s + parseFloat(r.purchase_price || 0), 0);
...
export function registerRealEstateTools(_registry: ToolRegistry, _config: PlaidPluginConfig): void {
