...
function createGetUserProfileHandler(config: PlaidPluginConfig): CapabilityHandler {
  return async (_input, _ctx) => {
    return withPool(config.databaseUrl, async (pool) => {
      const { rows } = await pool.query(
        `SELECT id, display_name, date_of_birth, gender,
...
function createGetHouseholdHandler(config: PlaidPluginConfig): CapabilityHandler {
  return async (_input, _ctx) => {
    return withPool(config.databaseUrl, async (pool) => {
      const { rows } = await pool.query(
...
function createGetFinancialGoalsHandler(config: PlaidPluginConfig): CapabilityHandler {
  return async (_input, _ctx) => {
    return withPool(config.databaseUrl, async (pool) => {
      const { rows } = await pool.query(
        `SELECT id, user_id, goal_type, description,
...
function createGetInvestmentPreferencesHandler(config: PlaidPluginConfig): CapabilityHandler {
  return async (_input, _ctx) => {
    return withPool(config.databaseUrl, async (pool) => {
      const { rows } = await pool.query(
        `SELECT id, user_id, risk_tolerance, liquidity_preference,
...
export function registerDemographicsTools(registry: ToolRegistry, config: PlaidPluginConfig): void {
  ...
  registry.register('get_user_profile', {
    ...
    description:
      "Get the primary user's demographic profile including name, date of birth, calculated age, " +
      'gender, state of residence, education, employment status, filing status, and estimated annual income.',
    parameters: { type: 'object', properties: {}, required: [] },
    alwaysEnabled: true,
    async execute() {
      return handler.userProfile({}, null) as Promise<Record<string, unknown>>;
    },
  });
  ...
  registry.register('get_household', {
    name: 'get_household',
    description:
      'Get all household members including spouse and children with their names, ' +
      'ages, relationships, and dates of birth.',
    parameters: { type: 'object', properties: {}, required: [] },
    alwaysEnabled: true,
    async execute() {
      return handler.household({}, null) as Promise<Record<string, unknown>>;
    },
  });
  ...
  registry.register('get_financial_goals', {
    ...
    description:
      'Get all active financial goals including retirement targets, education funding plans (529s), ' +
      'savings goals, and their priorities, target ages, and target amounts.',
    parameters: { type: 'object', properties: {}, required: [] },
    alwaysEnabled: true,
    async execute() {
      return handler.financialGoals({}, null) as Promise<Record<string, unknown>>;
    },
  });
  ...
  registry.register('get_investment_preferences', {
    ...
    description:
      'Get investment preferences including risk tolerance, liquidity preference, time horizon, ' +
      'preferred asset classes, avoided asset classes, and notes.',
    parameters: { type: 'object', properties: {}, required: [] },
    alwaysEnabled: true,
    async execute() {
      return handler.investmentPreferences({}, null) as Promise<Record<string, unknown>>;
    },
  });
...
export function registerDemographicsCapabilities(registry: CapabilityRegistry, config: PlaidPluginConfig): void {
  // 1. User profile
  registry.register({
